...
> 下一步若再压：~~①同款预取改造 Q8_0 路径~~（**已做，2026-06-11，见下**）②预取距离扫描（512B 未调优）③NPU 侧成为新主导（~45ms），CPU↔NPU overlap（B 线）价值更大了。
>
> **✅ Q8_0 路径也优化了（2026-06-11，commit 18d25b7）**：同款行内预取+向量累加移植到 Q8_0，实现为 kt-kernel
> 自包含 `kt_vec_dot_q8_0_q8_0`（裸 NEON），**ggml 原函数零改动 + `KT_Q8_REF=1` 安全回退/A-B 开关**，patch 0002 不动。
> 微基准 layer16 @128t 清净窗口：REF(ggml) 1.405ms/114 GB/s → KT 0.591ms/272 GB/s（**2.38×**，与 MXFP4 同量级——
> 推翻"Q8_0 受害轻一半"预判，行内预取是主因与块密度无关）；sig 跨 6 次运行逐位一致 = 复现 ggml 输出。
> **DSv4 生产不走此路（已用 MXFP4）**；价值在惠及其他只有 W8A8→Q8_0、走 CPU offload 的模型。字节仍是 MXFP4 的 2 倍，
> 故修到极致仍 ~2× 慢于 MXFP4。
>
